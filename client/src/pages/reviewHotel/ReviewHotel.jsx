...
const [selectedStayDuration, setSelectedStayDuration] = useState("Select one");
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post("http://localhost:3000/travelmate/addHotelReview", reviewData)
            ...
            {hotel ? <>
                <Container className="my-4">
                    <h1 className="text-center fw-bold mb-5 mt-4">
                        Tell us your experience at {hotel.hotelName}
                    </h1>
                    <Row>
                        <Col md={4} className="px-4">
                            <Card className="border border-0">
                                <Card.Img className='pt-4 px-4' variant="top" src="https://picsum.photos/500" alt="Hotel Marabedda" />
                                <Card.Body>
                                    <Card.Title className='px-2 fw-bold'>{hotel.hotelName}</Card.Title>
                        ...
                        <Col md={8}>
                            <Form onSubmit={handleSubmit}>
                                ...
                                <Form.Group>
                                    ...
                                    <div className="d-flex gap-1">
                                        {[...Array(5)].map((_, index) => (
                                            <span key={index} className="rating-circle" style={{ width: "25px", height: "25px", borderRadius: "50%", backgroundColor: (hoverRoomRating || roomRating) > index ? "#00A1FF" : "transparent", border: "2px solid #00A1FF", cursor: "pointer", }} onMouseEnter={() => handleMouseEnter(index + 1, "room")} onMouseLeave={() => handleMouseLeave("room")} onClick={() => handleRatingClick(index + 1, "room")} />
                                        ))}
                                    </div>
                                </Form.Group>
                                ...
                                <Form.Group>
                                    ...
                                    <div className="d-flex gap-1">
                                        {[...Array(5)].map((_, index) => (
                                            <span key={index} className="rating-circle" style={{ width: "25px", height: "25px", borderRadius: "50%", backgroundColor: (hoverLocationRating || locationRating) > index ? "#00A1FF" : "transparent", border: "2px solid #00A1FF", cursor: "pointer", }} onMouseEnter={() => handleMouseEnter(index + 1, "location")} onMouseLeave={() => handleMouseLeave("atmosphere")} onClick={() => handleRatingClick(index + 1, "location")} />
                                        ))}
                                    </div>
                                </Form.Group>
                                        ...
                                        <Form.Group>
                                            ...
                                            <div className="d-flex gap-2">
                                                {["Solo", "Family", "Friends", "Couple", "Business"].map((option) => (
                                                    <Button
                                                        key={option}
                                                        onClick={() => handleTravelTypeOptionClick(option)}
                                                        style={{
                                                            backgroundColor: selectedTravelTypeOption === option ? '#00A1FF' : 'transparent',
                                                            color: selectedTravelTypeOption === option ? 'white' : '#00A1FF',
                                ...
                                <Form.Group className="mt-3">
                                    ...
                                    <Form.Select value={selectedStayDuration} onChange={(e) => setSelectedStayDuration(e.target.value)}>
