...
const HospitalDataTable = () => {
  ...
  const [records, setRecords] = useState([]); // Stores all the hospital records
  const [filteredRecords, setFilteredRecords] = useState([]); // Stores filtered hospital records
  const [selectedRows, setSelectedRows] = useState([]);
   useEffect(() => {
      const fetchHospitals = async () => {
        try {
          const response = await fetch('http://localhost:3000/travelmate/allHospitals');
  ...
  function handleViewMore(row) {
    navigate(`/view-hospital/${row.id}`);
  }

  // Handle adding a new hospital (optional functionality)
  function handleAddHospital() {
    const newHospital = {
      name: `Hospital ${records.length + 1}`,
      description: `Description ${records.length + 1}`,
      available: true,
      rating: 3, // Default rating
      address: `Address ${records.length + 1}`,
    };

    setRecords([...records, newHospital]);
    setFilteredRecords([...filteredRecords, newHospital]); // Add new hospital to filtered records as well
  }
  ...
  async function handleDeleteSelected() {
    try {
      for (let row of selectedRows) {
        await fetch('http://localhost:3000/travelmate/deleteHospital', {
          method: 'DELETE',
          headers: {
